...
<template>
    ...
    <div class="flex flex-col space-y-6 p-6 sm:p-8">
        ...
        >
            ...
            <div class="grid gap-2">
                <Label for="foto_profil">Foto Profil <span class="text-muted-foreground font-normal">(Max 2MB)</span></Label>
                <Input
                    id="foto_profil"
                    type="file"
                    class="mt-1 block w-full"
                    name="foto_profil"
                />
                <div v-if="identitas?.foto_profil" class="mt-3 flex items-center gap-4 p-3 rounded-lg bg-muted/50 border">
                    <img :src="`/storage/${identitas.foto_profil}`" alt="Foto Profil" class="h-16 w-16 rounded-full object-cover border-2 border-border" />
                    <div class="space-y-0.5">
                        <p class="text-sm font-medium">Foto profil aktif</p>
                        <p class="text-xs text-muted-foreground">Upload foto baru untuk menggantinya</p>
                    </div>
                </div>
            ...
            <div class="grid gap-2">
                <Label for="cv_file">File CV <span class="text-muted-foreground font-normal">(PDF / DOC / DOCX, Max 5MB)</span></Label>
                <Input
                    id="cv_file"
                    type="file"
                    class="mt-1 block w-full"
                ...
                <div v-if="identitas?.cv_file" class="mt-2">
                    <a :href="`/storage/${identitas.cv_file}`" target="_blank" class="inline-flex items-center gap-1.5 text-sm font-medium text-blue-600 hover:text-blue-500 transition-colors">
